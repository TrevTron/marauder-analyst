...
void CommandLine::runCommand(String input) {
  ...
  if (cmd_args.get(0) == STOPSCAN_CMD) {
    if (file_serve_obj.running()) file_serve_obj.end();  // filesrv teardown
  ...
  // ls command
  else if (cmd_args.get(0) == FILESRV_CMD) {
    int k_sw = this->argSearch(&cmd_args, "-k");
    String key = (k_sw != -1 && cmd_args.size() > k_sw + 1) ? cmd_args.get(k_sw + 1) : "";
    if (file_serve_obj.running()) {
      file_serve_obj.end();
    } else {
      file_serve_obj.begin("MarauderFiles", "marauderfiles", key.c_str());
    }
  }
